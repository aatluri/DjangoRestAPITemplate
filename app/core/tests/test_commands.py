...
# may get thrown by the database depending on what
# stage of the start up process it is.
...
class CommandTests(SimpleTestCase):
    """Test commands."""
# Because we added a patch decorator at the top,
# we need to catch that in each of our test methods
    def test_wait_for_db_ready(self, patched_check):
        """Test waiting for database if database ready."""
        # When we call check inside our command, we just want
        # to return the value as TRUE.
        patched_check.return_value = True

        # Now we call the wait_for_db command. This will execute the
        # code inside the wait_for_db command.
        call_command('wait_for_db')

        # So this basically ensures that the mocked value here i.e the check
        # method inside our command, the mocked object,
        # which is the check method inside our command,
        # is called with these parameters.
        patched_check.assert_called_once_with(databases=['default'])

    # mock the sleep method.
    # So what's going to happen is we're going to
    # actually be checking database ]
    ...
    def test_wait_for_db_delay(self, patched_sleep, patched_check):
        ...
        # The first 2 times we call the mocked method we
        # want to raise the psycopsg2 error
